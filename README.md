...
12. **Alt+Enter cycles through window sizes.** The virtual canvas is 640×480; Alt+Enter steps the backbuffer through
    multiples of that (1×, 2×, 3×, and so on) up to whatever the current monitor can fit, then wraps back to 1×.
13. **Swimming is unrestricted by default.** Retail gates swimming(walking on water tiles) behind the `Swimming` skill,
    or having the GM flag to ignore collision. This client ships with that gate **off**, so any character walks/pathfinds
...
14. **Health bars, chants, and chat bubbles on creature sprites use a blended offset.** They sit halfway between a fixed
    baseline and the sprite's mean visible top instead of tracking frame heights. Small sprites sit a little higher than
...
buffered snapshot into typed events and routes them to UI elements. `InputBuffer` is a **static** class, so you can read
...
- `MouseX` / `MouseY` in **virtual** (640×480) coordinates. `SetVirtualScale(scale)` tells the buffer the backbuffer
  stretch factor so raw window coords get divided back down, using the same divisor for polled getters and per-event capture, so
...
- The dispatcher walks `Events` in order to (a) suppress a `TextInput` whose preceding `KeyDown` was consumed as a
  ...
  it fired, which matters for macros that chord modifiers with other keys inside a single frame.
...
1. **Phase 1, explicit focus.** If something has called `SetExplicitFocus(element)` (the built-in case is `UITextBox`,
   ...
   to its immediate parent panel as well, which is what lets a dialog close on Escape while the textbox eats all the
   other keystrokes.
...
- **From a screen**: put the logic in `WorldScreen.InputHandlers.cs` (or the equivalent in your own screen). That code
  ...
  against the static buffer snapshot. That's the right place for world-screen hotkeys like movement, casting, and pathfinding,
...
- **`CachedTexture2D`**: a `Texture2D` subclass whose `Dispose` is a no-op. Only the owning cache can release GPU memory,
  via `ForceDispose`. This lets callers hand the texture around freely without worrying about double-dispose.
...
Handles palette shimmer for tiles whose palette entries cycle through a color range, declared in the `mpt` / `stc` palette
files and made available via `PaletteLookup`. On map load it scans which tiles on the current map use cycling palettes,
...
`Chaos.Client.Rendering/`) because it walks `WorldState` to build the buffer, but its only readers are the
...
`GetTileOffsets`, and packs both together into a `LightSource` record. `Sources` then gives access to the filled region as a
...
publishing a unified buffer means both renderers agree on the set of light sources every frame, so there's no drift.
...
into a `stackalloc` buffer and ends in a single `.ToArray()`, with no intermediate `List<>`.
...
4. **The current-frame `LightSource` span from `LightingSystem`**, passed into `Update(camera, viewport, sources)` by
   ...
   It reads the span without keeping state of its own, and dirty-checks the incoming sources via `ComputeSourceHash` to skip rebuilds
...
`TabMapRenderer.Draw` takes the light source span from `LightingSystem` and a baseline offset array (radius 0, the player's
...
the visibility grid is rebuilt every frame from the span, moving lanterns and direction changes carry through automatically
...
`NoTabMap`, `SnowTileset`). Snow uses the `snowaNN.epf` sprite
...
handled here. `ColorCodesEnabled` is a per-call flag; widgets make it available as a property and pass it down.
...
> ```
>
> Windows and macOS ship fully self-contained binaries, with no extra install step.
...
2. Extend `PrefabPanel` in `Chaos.Client/Controls/` and use `CreateXxx` to create the children you care about.
...
`Chaos.Networking` and its dependencies, which this project consumes as a NuGet package, so you can't add new types to a
compiled dependency.
...
Other routes exist, such as republishing your own preview NuGets, or shimming new converters on the client side alongside the
