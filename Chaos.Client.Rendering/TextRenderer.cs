...
public static class TextRenderer
{
    ...
    /// <summary>Removes {=x} color codes from text (the TrueType path has no per-segment color, so it strips them). Strips
    ///     ANY "{=" + one char, even an unknown/invalid colour (e.g. from "/setchannelcolor x 1"), so a bad code never
    ///     shows as literal "{=?" garbage.</summary>
    ...
    public static List<string> WrapText(string text, int maxWidth)
    {
        ...
        foreach (var paragraph in paragraphs)
        {
            ...
            while (remaining.Length > 0)
            {
                ...
                //less than what we just used, otherwise FindLineBreak on the next loop pass
