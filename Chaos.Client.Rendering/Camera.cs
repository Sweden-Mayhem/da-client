...
public sealed class Camera
{
    ...
    /// </param>
    public (int MinX, int MinY, int MaxX, int MaxY) GetVisibleTileBounds(int mapWidth, int mapHeight, int extraMargin = 0, bool clampToMap = true)
    {
        ...
        //clampToMap:false returns the raw visible range (may extend past the map edges) - used by the
        //"extend background" debug option, which fills the off-map area with the clamped edge tile.
        if (clampToMap)
        {
