...
public static class ImageUtil
{
    ...
    /// <summary>
    ///     Returns a new texture where every non-transparent pixel is replaced with white (premultiplied), preserving the
    ///     original alpha. Drawing this over an icon at partial alpha produces a white highlight on only the icon's pixels.
    /// </summary>
    public static Texture2D BuildWhiteTinted(GraphicsDevice device, Texture2D source)
    {
        using var scope = new PixelBufferScope(source);

        for (var i = 0; i < scope.Count; i++)
        {
            var a = scope.Pixels[i].A;
            scope.Pixels[i] = new Color(a, a, a, a); // premultiplied white at original alpha
        }
