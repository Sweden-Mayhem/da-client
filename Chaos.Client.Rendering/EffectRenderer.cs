...
/// </summary>
/// <remarks>
///     The EffectTable (effect.tbl) sets the format: an entry of [0] indicates EFA; otherwise the entry lists EPF
...
public sealed class EffectRenderer : IDisposable
{
        ...
        float alpha = 1f)
    {
        ...
        //alpha < 1 fades the frame (premultiplied tint); used by the through-walls overlay pass so an effect shows
        //semi-transparent where a foreground tile would otherwise hide it
        batch.Draw(frame.Texture, screenPos, Color.White * alpha);
    ...
    {
        //check the effecttable to figure out the frame sequence and format
        ...
        float alpha = 1f)
    {
        ...
        batch.Draw(frame.Value.Texture, screenPos, Color.White * alpha);
    }
}
