...
public sealed class AislingRenderer : IDisposable
{
    ...
    /// <summary>
    ///     Renders the player's own front-facing aisling (body + equipment + hair) with an emote face/bubble overlaid on
    ///     it, exactly as the character looks in-world while emoting. Used by the emote menu so each icon is the real
    ///     character rather than a disembodied emote head. Returns the full <see cref="COMPOSITE_WIDTH" />x
    ///     <see cref="COMPOSITE_HEIGHT" /> composite (character centered with <see cref="LAYER_OFFSET_PADDING" /> of
    ///     side padding, all layers top-aligned); the caller crops to taste. The caller owns and disposes the texture.
    /// </summary>
    public Texture2D? RenderEmoteCharacter(in AislingAppearance appearance, int emotionFrame)
        => Render(in appearance, RIGHT_IDLE_FRAME, flipHorizontal: true, isFrontFacing: true, emotionFrame: emotionFrame);
