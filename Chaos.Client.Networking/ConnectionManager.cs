...
public sealed class ConnectionManager : IDisposable
{
    ...
    /// <summary>
    ///     The name of the logged-in character, filled in when <see cref="Login" /> is called.
    /// </summary>
    public string AislingName { get; private set; } = string.Empty;

    //session login credentials, kept in memory (never written to disk) so a silent reconnect after an unexpected
    //world disconnect can replay the login without prompting. Set every time Login() is called.
    private string LastLoginName = string.Empty;
    private string LastLoginPassword = string.Empty;
    private uint LastClientId1 = 1;
    private uint LastClientId2 = 1;

    /// <summary>
    ///     True once a successful login has stored this session's credentials, so an automatic reconnect can replay it.
    /// </summary>
    public bool CanReconnect => !string.IsNullOrEmpty(LastLoginName) && !string.IsNullOrEmpty(LastLoginPassword);

    /// <summary>
    ///     Re-sends this session's stored login credentials (see <see cref="CanReconnect" />). Used by the automatic
    ///     reconnect flow once the login server is reachable again. No-op if not in the Login state.
    /// </summary>
    public void ReLogin() => Login(LastLoginName, LastLoginPassword, LastClientId1, LastClientId2);
    ...
    /// <summary>
    ///     Custom build version sent at login so the server can reject out-of-date clients.
    ///     Set by the start screen from <c>GlobalSettings.CustomClientVersion</c>. Empty means do not send.
    /// </summary>
    public string CustomClientVersion { get; set; } = string.Empty;
    ...
    /// <summary>
    ///     True while a server redirect (lobby to login to world) is in flight. The old socket is intentionally closed
    ///     and a new one opened, so the brief Disconnected in between is expected, not a connection failure.
    /// </summary>
    public bool IsRedirecting => PendingRedirect is not null;
    ...
    private void FollowPendingRedirect()
    {
        ...
        //lobby to login uses empty keysaltseed (falls back to "default"), login to world uses character name
        ...
        uint clientId2 = 1)
    {
        ...
        //remember the credentials for this session so an unexpected world disconnect can silently reconnect
        LastLoginName = name;
        LastLoginPassword = password;
        LastClientId1 = clientId1;
        LastClientId2 = clientId2;

        //send our build version first so the login server can reject out-of-date clients
        //TCP keeps it ordered ahead of the login packet, so it is stored before OnLogin runs
        if (!string.IsNullOrEmpty(CustomClientVersion))
            Client.Send(
                new CustomClientVersionArgs
                {
                    Version = CustomClientVersion
                });
    ...
    /// <summary>
    ///     Uploads the local Friends list ("O" window) to the server so it can drive presence notifications
    ///     (friends-online on world entry, and "X has come online" pushes). Sent on world entry and whenever the
    ///     list is saved; an empty list is still sent so the server can notify others that this player came online.
    /// </summary>
    /// <param name="names">The friend names currently saved (the server caps at 20).</param>
    public void SendFriendList(IReadOnlyList<string> names)
        => SendIfWorld(
            new FriendListArgs
            {
                Names = names
    ...
    /// <summary>
    ///     Sends an item use request (equip, use).
    /// </summary>
    ...
    private void HandleRedirect(ServerPacket pkt)
    {
        ...
        //figure out target state based on current state
    ...
    private void HandleAttributes(ServerPacket pkt)
    {
        ...
        //merge partial updates with previously stored attributes so callers always get a complete picture
