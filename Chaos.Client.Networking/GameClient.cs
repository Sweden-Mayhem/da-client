...
public sealed class GameClient : IDisposable
{
    ...
    private static PacketSerializer BuildPacketSerializer()
    {
        ...
        // Override public-message converters: the NuGet defaults use 1-byte length (max 255 chars); these use
        // 2-byte length (max 65535). Must match the server-side converter change.
        converters[typeof(PublicMessageArgs)] = new LongPublicMessageConverter();
        converters[typeof(DisplayPublicMessageArgs)] = new LongDisplayPublicMessageConverter();

        // Override DisplayMenuConverter to read the custom ShowPlayerItems extension (grey items appended after slots)
        converters[typeof(DisplayMenuArgs)] = new SwmDisplayMenuConverter();
        ...
        //PacketSerializer must use the same 1252 encoding or high-byte characters desync between the two
        return new PacketSerializer(Encoding.GetEncoding(1252), converters);
    }
    #endregion
}
