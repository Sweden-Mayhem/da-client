...
public abstract class UIElement : IDisposable
{
    ...
    /// <summary>
    ///     Optional hover-tooltip text. When the cursor rests on this element (or a child that has none of its own), the
    ///     world's unified tooltip resolver shows this after the configured delay. Null/empty = no tooltip. Only works for
    ///     hit-test-visible elements (a pass-through panel never becomes the hover target, so set it on real controls).
    /// </summary>
    public string? Tooltip { get; set; }

    /// <summary>
    ///     Optional live tooltip text source, evaluated each time the tooltip is shown. Takes precedence over
    ///     <see cref="Tooltip" /> when set and non-empty, so a value that changes at runtime (e.g. a menu entry showing the
    ///     action's CURRENT hotkey, which the player can rebind) always reflects the latest state instead of a string baked
    ///     in once at construction.
    /// </summary>
    public Func<string?>? TooltipProvider { get; set; }
    ...
    /// <summary>
    ///     When true, the parent panel's draw loop skips this element, so its owner can draw it explicitly at a different
    ///     point in the frame. Used for the item tooltip: it must paint AFTER the dialog's native-resolution text (which
    ///     is drawn on top of the whole Root pass), so it stays in Root for layout/state but is drawn last by the owner.
    ///     Layout, hit-testing and Update are unaffected.
    /// </summary>
    ...
    /// <summary>The element's current clip rectangle (native screen space), as computed by the last Draw. Made available so the
    ///     native-text pass can clip RenderNative glyphs to the same region the in-place path uses (e.g. a scroll viewport),
    ///     instead of drawing them past the gadget edge.</summary>
    public Rectangle CurrentClipRect => ClipRect;
    ...
    public virtual bool ContainsPoint(int screenX, int screenY) =>
