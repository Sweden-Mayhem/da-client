...
public sealed class SpellBookPanel : PanelBase
{
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        //read cooldown state each frame, progressive draining meter (same as skills) with the remaining seconds on the icon
        for (var i = 0; (i < VisibleSlotCount) && (i < Slots.Count); i++)
        {
            var slot = (byte)(i + SlotOffset + 1);
            var control = Slots[i];

            control.CooldownPercent = WorldState.SpellBook.GetCooldownPercent(slot);
            control.CooldownSeconds = WorldState.SpellBook.GetCooldownRemainingSeconds(slot);
        }
    }
}