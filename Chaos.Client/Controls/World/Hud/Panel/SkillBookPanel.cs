...
public sealed class SkillBookPanel : PanelBase
{
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        //read cooldown state each frame, the progressive draining meter draws over the plain icon (no pre-tinted copies)
        for (var i = 0; (i < VisibleSlotCount) && (i < Slots.Count); i++)
        {
            var slot = (byte)(i + SlotOffset + 1);
            var control = Slots[i];

            control.CooldownPercent = WorldState.SkillBook.GetCooldownPercent(slot);
            control.CooldownSeconds = WorldState.SkillBook.GetCooldownRemainingSeconds(slot);
        }
    }
}