...
public sealed class WorldHudControl : PrefabPanel, IWorldHud
{
    ...
    /// <summary>The control prefab set this HUD was built from, so standalone hotbars can reuse the same art.</summary>
    public ControlPrefabSet HudPrefabSet => PrefabSet;
    ...
    /// <summary>
    ///     Removes a tab panel from this HUD (its children and tab registry) and returns it, so it can be
    ///     re-hosted in the new windowed UI while keeping all of its data-binding and slot-click wiring.
    ///     ShowTab no longer touches a detached panel, so the new host fully controls its visibility.
    /// </summary>
    public UIPanel? DetachTab(HudTab tab)
    {
        var panel = TabPanels[(int)tab];

        if (panel is null)
            return null;

        Children.Remove(panel);
        TabPanels[(int)tab] = null;
        panel.X = 0;
        panel.Y = 0;
        panel.Visible = true;
