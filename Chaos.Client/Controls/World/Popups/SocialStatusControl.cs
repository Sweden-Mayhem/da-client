...
public sealed class SocialStatusControl : PrefabPanel
{
        ...
        : base("lemot")
    {
        ...
        //crisp TTF at native res via WorldScreen's generic menu-text pass (this picker is hosted in a magnifier)
        if (DescriptionLabel is not null)
        {
            DescriptionLabel.Native(11);
            DescriptionLabel.RenderNative = true;
            //nudge the status word up 1px from the prefab slot and give the box 3px more height so it sits centered
            DescriptionLabel.Y -= 1;
            DescriptionLabel.Height += 3;
        }
