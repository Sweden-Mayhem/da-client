...
///     Buttons: Prev, Next, New, Delete, Up (back to list), Close. Hosted in a draggable ScaleHost (a free-floating window
///     like Group/Equipment): follows the Window size scale, opens centered with no animation, and is dragged by its
///     background art (the header value labels are non-hit-test so the header band drags; the body text stays selectable).
/// </summary>
public sealed class ArticleReadControl : PrefabPanel, INativeTextDrawer
{
        ...
        : base("_narti", false)
    {
        ...
        if (PrevButton is not null)
            PrevButton.Tooltip = "Previous\nRead the previous post on this board.";

        if (NextButton is not null)
            NextButton.Tooltip = "Next\nRead the next post on this board.";
    ...
    //the player's walk-up/down keys (W/S AND the arrows, whatever they're bound to) scroll the post body. Polled from
    ...
    {
        //bound board key backs out one level (to the article list); Escape closes the whole menu
