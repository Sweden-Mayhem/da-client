...
public sealed class SelfProfileFamilyTab : PrefabPanel
{
        ...
        : base(prefabName, false)
    {
        ...
        //the book sits in a magnifying ScaleHost, so paint labels + the editable family fields in crisp native TTF
        //one size smaller (10) than the other profile tabs, the family fields read better small
        SelfLabel = CreateLabel("Self")?.Native(10);
        SelfLabel?.Text = WorldState.PlayerName;
        SelfLabel?.HorizontalAlignment = HorizontalAlignment.Center;

        FamilyLabel = CreateLabel("Family")?.Native(10);

        for (var i = 0; i < 10; i++)
        {
            TextFields[i] = CreateTextBox($"Text{i}")?.Native(10);
