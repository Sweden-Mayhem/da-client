...
public sealed class SelfProfileTabControl : PrefabPanel
{
    ...
    //hover help for each tab, same order as TabControlNames (first line = title, rest = body)
        ...
        : base("_nui", false)
    {
        ...
        for (var i = 0; i < TAB_COUNT; i++)
        {
            ...
            tabBtn.Tooltip = TabTooltips[i];

            //the prefab's image is the big/selected state, so move it to selectedtexture
            ...
            //set initial selection state, the active tab suppresses redundant click sounds
            tabBtn.IsSelected = tab == ActiveTab;
            tabBtn.SuppressClickSound = tab == ActiveTab;
    ...
    private PrefabPanel? CreateTabPage(StatusBookTab tab)
    {
        ...
        //pass-through so empty/decorative areas of the page fall through to the draggable book host.
        //Interactive children (slots, list rows, scrollbars, buttons, clickable text) stay hit-testable,
        //so this only changes what happens when a click lands on the page's own background.
        page.IsPassThrough = true;
    ...
    /// <summary>Shows/hides the equipment book's level-up arrows from the player's unspent attribute-point count.</summary>
    public void SetUnspentStatPoints(int count) => GetOrCreateEquipmentPage()?.SetUnspentPoints(count);

    /// <summary>The equipped slot under a book-space point (for the item tooltip), or null.</summary>
    public EquipmentSlot? HitEquipItem(int x, int y) => GetOrCreateEquipmentPage()?.HitItemSlot(x, y);

    /// <summary>The stat whose value label is under a book-space point (for the info tooltip), or null.</summary>
    public StatInfoKind? HitEquipStat(int x, int y) => GetOrCreateEquipmentPage()?.HitStatInfo(x, y);

    /// <summary>The info hotspot (over a baked field-name word) under a book-space point, or null.</summary>
    public InfoHotspot? HitEquipInfoHotspot(int x, int y) => GetOrCreateEquipmentPage()?.HitInfoHotspot(x, y);
    ...
    public void SwitchTab(StatusBookTab tab)
    {
        ...
        if ((oldIndex < TAB_COUNT) && TabButtons[oldIndex] is not null)
        {
            ...
            TabButtons[oldIndex]!.SuppressClickSound = false;
        }
        ...
        if ((newIndex < TAB_COUNT) && TabButtons[newIndex] is not null)
        {
            ...
            TabButtons[newIndex]!.SuppressClickSound = true;
        }
    ...
    /// <summary>Closes the book and fires <see cref="OnClose" /> (which saves the family list), the same path as the
    ///     close (X) button and Escape. Used by the Equipment/Legend hotkeys to toggle the book shut.</summary>
    public void Close()
    {
        Hide();
        OnClose?.Invoke();
    ...
    private void OnAttributesChanged()
    {
        ...
        SetUnspentStatPoints(attrs.UnspentPoints);
    }
    ...
    #region Skills API
    /// <summary>
    ///     Fills the Skills tab with parsed ability metadata.
    /// </summary>
