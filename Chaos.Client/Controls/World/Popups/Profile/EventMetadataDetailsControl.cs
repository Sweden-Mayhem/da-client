...
public sealed class EventMetadataDetailsControl : PrefabPanel
{
        ...
        : base("_nui_eve")
    {
        ...
        MustLabel = CreateLabel("MUST")?.Native(11);
        MustLabel?.ForegroundColor = LegendColors.White;
        RewardLabel = CreateLabel("REWARD")?.Native(11);
    ...
    /// <summary>
    ///     Populates and shows the detail view for the given event entry. Placement + magnification are owned by the
    ///     wrapping ScaleHost (centered + scaled by "Window size" on show), so this just fills in and shows the content.
    /// </summary>
    public void ShowEntry(EventMetadataEntry entry, EventState state)
    {
