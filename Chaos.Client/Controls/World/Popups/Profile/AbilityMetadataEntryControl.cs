...
public sealed class AbilityMetadataEntryControl : PrefabPanel
{
        ...
        : base("_nui_ski", false)
    {
        ...
        NameLabel = CreateLabel("NAME")?.Native(10);
        ...
        //nudge both lines up 2px and give them a few extra pixels of clip height so descenders ("g","y") aren't cut
        foreach (var lbl in new[] { NameLabel, LevelLabel })
            if (lbl is not null)
            {
                lbl.Y -= 2;
                lbl.Height += 3;
