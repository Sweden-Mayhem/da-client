...
public sealed class GroupTabControl : PrefabPanel
{
        ...
        : base("_ngcmain", false)
    {
        ...
        if (Tab0Button is not null)
        {
            ...
            Tab0Button.Tooltip = "Members\nThe players in your group and their health. As the leader, you can remove a member from here.";
        }
        ...
        if (Tab1Button is not null)
        {
            ...
            Tab1Button.Tooltip = "Recruitment\nAdvertise your group so others can find and join it. Set a title, level range, and how many of each class you want.";
        }
        ...
        //pass-through so empty/decorative areas of either tab page fall through to the draggable group host.
        //Only these wrapped instances get it: the standalone GroupBoxViewer (also a GroupRecruitPanel) is not
        //in a host, so making the type itself pass-through there would leak clicks to the world behind it.
        MembersPanel.IsPassThrough = true;
        RecruitPanel.IsPassThrough = true;
    ...
    ///     a self-ViewGroupBox query (when HasActiveGroupBox is true) so the panel fills from
    ...
    /// <summary>
    ///     Shows the window on the recruit tab in owner-edit mode, filled from server data.
    /// </summary>
    ...
    /// <summary>
    ///     Shows the window on the recruit tab in viewer mode, filled from server data.
    /// </summary>
