...
public sealed class CharacterCreationControl : PrefabPanel
{
    ...
    //text fields are type 7 with 0 images, manually created
        ...
        : base("_ncreate", false)
    {
        ...
        PasswordField = CreateTextBox("PASSWD", 24);
        PasswordConfirmField = CreateTextBox("PASSWD2", 24);
        NameField?.Native(11);
        PasswordField?.Native(11);
        PasswordConfirmField?.Native(11);
        ...
        //gender toggles use hover images, not button press behavior
        //create as uibuttons to get textures and position, then use custom draw logic
        ...
        //character preview centered in the custom art's circle (the retail HUMAN rect does not match it)
        PreviewX = 257;
        PreviewY = 165;
        PreviewWidth = 120;
        PreviewHeight = 120;
        ...
        //lay the functional controls onto the custom "New Character" art (cc_bg.png, 640x480)
        //input fields over the painted boxes (box left ~28, tops ~135/210/282, ~32 tall)
        //UITextBox top-aligns its text, so PaddingTop centers the 12px line in the box
        PlaceField(NameField, 136);
        PlaceField(PasswordField, 206);

        //PASSWD2 mirrors PASSWD's box + text layout exactly (same PlaceField setup); only its Y differs
        PlaceField(PasswordConfirmField, 274);

        if (NameField is not null)
            NameField.Width = 100;

        // Constrain password fields to 15 visible characters (15 × 6px advance = 90px + 2px margin).
        // Text scrolls horizontally for chars beyond the visible area; MaxLength still allows up to 24.
        if (PasswordField is not null)
            PasswordField.Width = 100;

        if (PasswordConfirmField is not null)
            PasswordConfirmField.Width = 100;

        //gender toggles (drawn by this panel) in the upper half of the circle
        CenterOn(MaleToggleArea, 283, 135);
        CenterOn(FemaleToggleArea, 351, 135);

        //character rotation arrows flanking the preview
        CenterOn(AngleLeftButton, 228, 215);
        CenterOn(AngleRightButton, 406, 215);

        //hairstyle arrows flanking the painted "Hair" label
        CenterOn(HairLeftButton, 262, 287);
        CenterOn(HairRightButton, 379, 287);

        //hair color palette centered under "Hair" (UIImage draws at the texture's own size, so center via X)
        if (SwatchImage is not null)
        {
            SwatchImage.X = 318 - SwatchImage.Width / 2;
            SwatchImage.Y = 302;
        }
        ...
        static void PlaceField(UITextBox? f, int y)
        {
            ...
            f.Width = 128;
            f.Height = 32;
            f.PaddingTop = 10; //(32 - 12) / 2 -> the line sits vertically centered in the box
        }

        static void CenterOn(UIElement? e, int cx, int cy)
        {
            if (e is null)
                return;
    ...
    public override void OnKeyDown(KeyDownEvent e)
    {
        ...
        {
            case Keys.Tab:
                //cycle focus from name to password to passwordconfirm and back to name
