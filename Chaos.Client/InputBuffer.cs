...
public static class InputBuffer
{
    ...
    //raw-pixel origin of the virtual area. Non-zero for letterboxed legacy screens (the 640x480 target is best-fit
    //into WorldDrawRect, so the bars shift the origin); subtracted before scaling so clicks land on the right pixel.
    private static int VirtualOriginX;
    private static int VirtualOriginY;
    ...
    //each frame, and Update() scans it once to fill the query-style frame
    ...
    //the SDL event watcher callback, must be held in a static field so the GC doesn't
    ...
    /// <summary>Raw window-pixel cursor position (before the virtual-screen transform). Use when hit-testing against
    ///     screen-space geometry such as the lobby's native-text link rects, where the virtual mapping doesn't apply.</summary>
    public static int RawMouseScreenX => RawMouseX;

    /// <summary>Raw window-pixel cursor Y. See <see cref="RawMouseScreenX" />.</summary>
    public static int RawMouseScreenY => RawMouseY;
    ...
    private static int ToVirtualX(int raw) => (int)((raw - VirtualOriginX) / VirtualScaleX);
    private static int ToVirtualY(int raw) => (int)((raw - VirtualOriginY) / VirtualScaleY);
    ...
    /// <summary>
    ///     Sets the raw→virtual transform: <paramref name="scaleX" />/<paramref name="scaleY" /> per axis and the raw-pixel
    ///     origin (<paramref name="originX" />/<paramref name="originY" />) of the virtual area. For legacy screens the
    ///     640×480 target is best-fit (letterboxed) into <c>WorldDrawRect</c>, so the origin is the bar offset and the
    ///     scale is the best-fit ratio - subtracting the origin first makes a click land on the correct pixel.
    /// </summary>
    public static void SetVirtualTransform(float scaleX, float scaleY, int originX, int originY)
    {
        VirtualScaleX = scaleX;
        VirtualScaleY = scaleY;
        VirtualOriginX = originX;
        VirtualOriginY = originY;
    }
    ...
    ///     game update before any caller reads the query surface or the event stream.
    /// </summary>
    ...
    public static void Update(bool isActive)
    {
        ...
        //freeze the unified event stream and figure out the query-style frame snapshot
    ...
    private static void HandleMouseButtonEvent(nint sdlEvent, bool isPress)
    {
        ...
        var mouseButton = sdlButton switch
        {
            ...
            Sdl.BUTTON_X1     => MouseButton.X1,
            Sdl.BUTTON_X2     => MouseButton.X2,
            _                 => (MouseButton)(-1)
        };
    ...
    //ignored, callers only want integer vertical notches.
