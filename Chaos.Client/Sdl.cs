...
internal static partial class Sdl
{
    ...
    public const byte BUTTON_X1 = 4;
    public const byte BUTTON_X2 = 5;
    ...
    [LibraryImport("SDL2")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial void SDL_SetWindowMinimumSize(nint window, int minW, int minH);
    ...
    //SDL_WindowFlags bits reported by SDL_GetWindowFlags (used by ChaosGame for maximize detection)
    public const uint SDL_WINDOW_MAXIMIZED = 0x00000080;

    //SDL_Init subsystem flag for audio (used by SoundSystem during mixer bring-up)
    ...
    //clipboard via SDL2, the SAME clipboard the SDL window participates in, so copy/paste integrates with the OS
    //selection on the actual runtime (Windows under Wine / X11 / Wayland). Used by Utilities.Clipboard.
    [LibraryImport("SDL2", StringMarshalling = StringMarshalling.Utf8)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial int SDL_SetClipboardText(string text);

    //returns a freshly malloc'd UTF-8 C string the CALLER must release with SDL_free (empty string when nothing is set).
    [LibraryImport("SDL2")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    ...
    /// <summary>Reads the system clipboard via SDL, freeing SDL's returned buffer. Empty string when unset.</summary>
