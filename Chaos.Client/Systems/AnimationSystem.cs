...
public static class AnimationSystem
{
    ...
    private const float DEFAULT_WALK_FRAME_MS = 100f;   // local player walk pace, 100ms per frame is 400ms per tile
    ...
    internal const float MIN_WALK_DURATION_MS = 75f;

    //one iso tile step is (28, 14) px in either sign
    //a leftover walk offset larger than this is a desync so the carry-over in StartWalk drops it
    private const float ONE_TILE_LENGTH_SQUARED = (28f * 28f) + (14f * 14f);

    //the server's animation speed value times 10 gives the total animation duration in ms
    //a smaller speed means a faster animation (speed 25 is 250ms, speed 100 is 1000ms)
    ...
    //fallback per-frame delay for idle animation in ms, used by aislings and uninitialized entities
    //creature intervals come directly from CreatureAnimInfo.AnimationIntervalMs
    private const float DEFAULT_IDLE_FRAME_MS = 300f;

    //aisling walk anim "01" frames, 0 is up idle, 1-4 up walk, 5 is right idle, 6-9 right walk
    ...
    {
        //a step that begins while the previous slide is still playing continues from where the sprite
        //actually is instead of snapping to the grid point, which keeps fast walking smooth
        //we re-base the new slide's start offset by the leftover offset so screen position stays continuous
        //a leftover larger than one tile is a desync and gets dropped, snapping is better than a long crawl
        var carryOver = Vector2.Zero;

        if (entity.AnimState == EntityAnimState.Walking)
        {
            var residual = entity.VisualOffset;

            if (residual.LengthSquared() <= ONE_TILE_LENGTH_SQUARED)
                carryOver = residual;
        }

        //facing direction always matches walk direction, set it here so direction and WalkStartOffset
        //stay locked together and the sprite never faces one way while the screen scrolls another
        ...
        entity.WalkStartOffset = GetWalkOffset(direction) + carryOver;
    ...
    public static void StartBodyAnimation(WorldEntity entity, BodyAnimation bodyAnim, ushort animSpeed)
    {
        ...
        if (DataUtilities.IsEmote(bodyAnim))
        {
            ...
            //non-emote body animations, animspeed times 10 gives the per-frame interval
    ...
    /// </summary>
    /// <remarks>
    ///     The <see cref="BodyAnimation" /> is mapped to an attack index (Kick to Attack2, RoundHouseKick to Attack3,
    ///     everything else to Attack1) and falls back to Attack1 when the mapped attack has no frames. Per-frame display time
    ...
    public static void Advance(WorldEntity entity, float elapsedMs, bool smoothScroll = false)
    {
        ...
        //aislings use idleanimframecount from the "04" epf, creatures always tick
    ...
    private static void AdvanceWalk(WorldEntity entity, float elapsedMs, bool smoothScroll)
    {
        ...
        //some entities have no walk frames and that's fine, the walk slide still plays out
        ...
        //both smooth and stepped offsets stay on the integer 2:1 isometric lattice (28, 14
        //and the even-x points between) so the diagonal scroll never wobbles off its slope
        if (smoothScroll)

            //smooth, slide toward the destination by the elapsed fraction of the step, snapped to
            //the nearest lattice point. unlike the stepped path this advances one pixel at a time
            //so the scroll is as smooth as the low-res world can show at any framerate
            entity.VisualOffset = GetSmoothWalkOffset(entity.WalkStartOffset, 1f - progress);
        else
    ...
    ///     current state. IsFrontFacing controls layer draw order (front vs back).
    /// </summary>
    ...
    private static (int FrameIndex, bool Flip) GetCreatureIdleFrame(WorldEntity entity, in CreatureAnimInfo info)
    {
        ...
        //even when StandingFrameCount is greater than 0, because no idle loop is defined
        ...
        //single-direction sprite, second direction frames don't exist so reuse base frames for all directions
    ...
    private static Vector2 GetSteppedWalkOffset(Vector2 startOffset, int frameIndex, int frameCount)
    {
        ...
        //(GetWalkOffset returned Vector2.Zero), there is no slope to follow so short-circuit
    ...
    /// <summary>
    ///     Returns the walk offset at a continuous point in the slide, snapped to the isometric 2:1 lattice.
    ///     <paramref name="remaining" /> is 1 at the start of the step (offset == <paramref name="startOffset" />) and 0 at
    ///     the destination (offset == zero). Used by smooth scrolling so the camera advances a pixel at a time rather than
    ///     in a handful of discrete frame steps.
    /// </summary>
    private static Vector2 GetSmoothWalkOffset(Vector2 startOffset, float remaining)
    {
        var startX = (int)startOffset.X;
        var startY = (int)startOffset.Y;

        //a zero start offset means the direction did not map to a cardinal walk vector
        //(GetWalkOffset returned Vector2.Zero), no slope to follow so short-circuit before the
        //y = x * startY / startX division would throw
        if (startX == 0)
            return Vector2.Zero;

        //round the remaining distance to a whole pixel, then to the nearest even number (toward zero)
        //so y = x/2 stays exact and every value lands on the 2:1 slope, same lattice as the stepped
        //path, just sampled continuously instead of at a fixed number of frames
        var x = (int)MathF.Round(startX * remaining);
