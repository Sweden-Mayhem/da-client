...
public sealed class CastingSystem
{
    ...
    /// <summary>The name of the spell currently being chanted (for the cast-progress bar), or null when not chanting.</summary>
    public string? ChantSpellName => ChantingSlot?.AbilityName;

    /// <summary>0..1 progress of the current chant. Each cast line takes <see cref="CHANT_INTERVAL_MS" />, so the whole
    ///     chant runs for CastLines seconds. This is elapsed/total, used to draw the cast bar. 0 when not chanting.</summary>
    public float ChantProgress
    {
        get
        {
            ...
            //elapsed is whole lines already sent ((LineIndex-1) of them) plus the partial second toward the next
            var elapsed = ((LineIndex - 1) * CHANT_INTERVAL_MS) + ElapsedMs;
            var total = castLines * CHANT_INTERVAL_MS;
