...
///     in by eye. Not persisted. The defaults here are the real shipped values and the window just lets us find good
...
public static class DebugSettings
{
    ...
    // ── Light vertical reach: a light lights front (greater-depth) foreground proportional to how high it sits ──
    //reach in iso-depth steps = clamp(round(-OffsetY / PxPerStep), 0, MaxSteps). A ground light (OffsetY 0) reaches 0
    //(own depth only). A raised light reaches 1 step per PxPerStep px of height but never more than MaxSteps. The cap is
    //what stops a high-hanging lamp from lighting tall sprites (trees) several tiles in front of it
    ...
    public static float LightFlickerScale = 1.7f;    //1 = subtle 0.1 amplitude, higher = a more visible flicker
    ...
    // ── Tile-light turn-on (each lamp fades on/off, with a slight position-seeded stagger) ──
